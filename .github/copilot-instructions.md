...
- **Classe principale** : `VoirieCommunale` dans `voirie_communale.py` gère le cycle de vie du plugin (initGui, unload, run)
- **Interface utilisateur** : Séparation entre la logique (`voirie_communale_dialog.py`) et l'UI Qt Designer (`.ui`)
...
pyuic5 voirie_communale_dialog_base.ui -o voirie_communale_dialog_base.py
```
...
- **Logs** : Utilisez `QgsMessageLog.logMessage("message", "VoirieCommunale", Qgis.Info)`
...
New-Item -ItemType SymbolicLink -Path $pluginDir -Target "d:\voirie_communale"
```
...
Copy-Item -Recurse -Force D:\voirie_communale $pluginDir
```
...
- **UI compilé** : Ne pas modifier `voirie_communale_dialog_base.py` manuellement, toujours régénérer depuis le .ui
- **Ressources** : Référencer via `:/plugins/voirie_communale/` dans le code après compilation
...
- Icône référencée depuis les ressources compilées : `:/plugins/voirie_communale/icon.png`
...
python -c "from voirie_communale import VoirieCommunale; print('OK')"
